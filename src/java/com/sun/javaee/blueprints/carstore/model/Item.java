...
public class Item implements java.io.Serializable {
    ...
    public String[] validateWithMessage() {
        ...
        if(name == null || name.equals("")) {
            valMess.add(CarstoreUtil.getMessage("invalid_item_name"));
        }
        ...
        if(description == null || description.length() < 1 || description.indexOf("<script") > -1 || description.indexOf("<link") > -1) {
            valMess.add(CarstoreUtil.getMessage("invalid_item_description"));
        }

        // make sure price is a number
        if(price.intValue() < 0) {
            valMess.add(CarstoreUtil.getMessage("invalid_item_price"));
        }

        if(imageURL == null || (!imageURL.endsWith(".jpg") && !imageURL.endsWith(".gif") && imageURL.endsWith(".png"))) {
            // not a proper upload so error
           valMess.add(CarstoreUtil.getMessage("invalid_item_imageurl"));
        }
