...
package com.sun.javaee.blueprints.carstore.controller.actions;

import com.sun.javaee.blueprints.carstore.controller.ControllerAction;
    ...
    private String handleItems(HttpServletResponse response, List<Item> items, String baseURL, String format, String callback) {
        ...
        if(format != null && (format.toLowerCase().equals("json") || format.toLowerCase().equals("jsonp"))) {
            ...
            for (Item i : items) {
                sb.append("{");
                sb.append("\"productID\":\"");
                sb.append(CarstoreUtil.encodeJSONString(i.getProductID()));
                sb.append(DOUBLE_QUOTE);
                sb.append(COMMA);

                sb.append("\"itemID\":\"");
                sb.append(CarstoreUtil.encodeJSONString(i.getItemID()));
                sb.append(DOUBLE_QUOTE);
                sb.append(COMMA);

                sb.append("\"name\":\"");
                sb.append(CarstoreUtil.encodeJSONString(i.getName()));
                sb.append(DOUBLE_QUOTE);
                sb.append(COMMA);

                sb.append("\"description\":\"");
                //sb.append("\"description\":\"<script>alert(\\'test\\');</script>");
                //sb.append("\"description\":\"<span onmouseover=\\'alert(document.cookie);\\'>" + CarstoreUtil.encodeJSONString(i.getDescription()) + " </span>");
                sb.append(CarstoreUtil.encodeJSONString(i.getDescription()));
                sb.append(DOUBLE_QUOTE);
                sb.append(COMMA);

                sb.append("\"price\":\"");
                sb.append(CarstoreUtil.encodeJSONString(String.valueOf(i.getPrice())));
                sb.append(DOUBLE_QUOTE);
                sb.append(COMMA);

                sb.append("\"imageThumbURL\":\"");
                sb.append(CarstoreUtil.encodeJSONString(i.getImageThumbURL()));
                sb.append(DOUBLE_QUOTE);
                sb.append(COMMA);


                sb.append("\"imageURL\":\"");
                sb.append(CarstoreUtil.encodeJSONString(i.getImageURL()));
                sb.append(DOUBLE_QUOTE);
                sb.append("}");
                sb.append(COMMA);
            }
    ...
    private String handleCategories(HttpServletResponse response, String format, String callback){
        ...
        if (format != null && (format.toLowerCase().equals("json") ||  format.toLowerCase().equals("jsonp"))) {
            ...
            for (Category c : cf.getCategories()) {
                ...
                sb.append("{");
                sb.append("\"id\":\"" + CarstoreUtil.encodeJSONString(c.getCategoryID()) + "\",");
                sb.append("\"catid\":\"" + CarstoreUtil.encodeJSONString(catid) + "\",");
                sb.append("\"name\":\"" + CarstoreUtil.encodeJSONString(c.getName()) + "\",");
                sb.append("\"description\":\"" + CarstoreUtil.encodeJSONString(c.getDescription()) + "\",");
                sb.append("\"imageURL\":\"" + CarstoreUtil.encodeJSONString(c.getImageURL()) + "\",");
                sb.append("\"products\": [");
                ...
                for (Product p : cf.getProducts(catid)) {
                    ...
                    sb.append("{");
                    sb.append("\"id\":\"" + CarstoreUtil.encodeJSONString(p.getProductID()) + "\",");
                    sb.append("\"catid\":\"" + CarstoreUtil.encodeJSONString(catid) + "\",");
                    sb.append("\"name\":\"" + CarstoreUtil.encodeJSONString(p.getName()) + "\",");
                    sb.append("\"description\":\"" + (p.getDescription()) + "\",");
                    sb.append("\"imageURL\":\"" + CarstoreUtil.encodeJSONString(p.getImageURL()) + "\"");
                    sb.append("}");
                }
                sb.append("]");
                sb.append("}");
