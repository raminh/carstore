...
package com.sun.javaee.blueprints.carstore.controller;
...
import com.sun.javaee.blueprints.carstore.util.CarstoreUtil;
import com.sun.javaee.blueprints.carstore.util.CarstoreConstants;
import com.sun.javaee.blueprints.carstore.util.ImageScaler;
...
import com.sun.javaee.blueprints.carstore.proxy.GeoCoder;
import com.sun.javaee.blueprints.carstore.proxy.GeoPoint;

import com.sun.javaee.blueprints.carstore.model.Address;
import com.sun.javaee.blueprints.carstore.model.CatalogFacade;
import com.sun.javaee.blueprints.carstore.model.Category;
import com.sun.javaee.blueprints.carstore.model.Product;
import com.sun.javaee.blueprints.carstore.model.FileUploadResponse;
import com.sun.javaee.blueprints.carstore.model.Item;
import com.sun.javaee.blueprints.carstore.model.Tag;
import com.sun.javaee.blueprints.carstore.model.SellerContactInfo;
...
public class FileUploadBean {
        ...
        try {
            ...
            try {
                ...
                if(fileNameKey == null) {
                    ...
                    sendErrorResponse(response, writer, CarstoreUtil.getMessage("invalid_item_imageurl"));
                    return;
                }
                ...
                if(proxyHost != null && !proxyHost.equals("") && proxyPort != null && proxyPort.equals("")) {
                    ...
                    CarstoreUtil.getLogger().log(Level.INFO, "Setting proxy to " + proxyHost + ":" + proxyPort + ".  Make sure server.policy is updated to allow setting System Properties");
                    ...
                    CarstoreUtil.getLogger().log(Level.INFO, "A \"proxyHost\" and \"proxyPort\" isn't set as a web.xml context-param. A proxy server may be necessary to reach the open internet.");
                }
                ...
                if(totAddr.length() > 0) {
                    try {
                        ...
                        if ((points == null) || (points.length < 1)) {
                            CarstoreUtil.getLogger().log(Level.INFO, "No addresses for location - " + totAddr);
                        } else if(points.length > 1) {
                            CarstoreUtil.getLogger().log(Level.INFO, "Matched " + points.length + " locations, taking the first one");
                        }
                        ...
                        CarstoreUtil.getLogger().log(Level.WARNING, "geocoder.lookup.exception", ee);
                    }
                }
                BigDecimal pricex;
                try {
                    ...
                    CarstoreUtil.getLogger().log(Level.INFO, "Price isn't in a proper number - " + price);
                }
                ...
                CarstoreUtil.getLogger().log(Level.FINE, "Item " + name + " has been persisted");
                ...
                CarstoreUtil.getLogger().log(Level.SEVERE, "fileupload.persist.exception", ex);
            ...
            CarstoreUtil.getLogger().log(Level.SEVERE, "FileUploadPhaseListener error writting AJAX response", iox);
        }
    }
    ...
    private String constructThumbnail(String path) {
        ...
        try {
            ...
            CarstoreUtil.getLogger().log(Level.SEVERE, "ERROR in generating thumbnail", e);
        }
        return thumbPath;
    }
    ...
    public String getUploadImageDirectory() {
        return CarstoreConstants.CARSTORE_IMAGE_DIRECTORY + "/images";
    }
